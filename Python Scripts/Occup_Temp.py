...
occupancy_packet = '000-0\n'
temp_packet = '000-11.12\n'
...
def read_and_transmit_to_GUI():
    while True:
        ...
        if len(received_string) == len(occupancy_packet) and received_string[3] == '-':
            ...
            node_data[2] = node_state  # State of the selected node
            ...
            with open(log_file_name, 'a') as log_file:
                log_file.write(
                    'Room: ' + str(node_data[1]) + ', Node number: ' + str(node_data[0]) + ', ' + str(node_data) + '\n')
            '''

        #Parse string: sample_packet = '000-0\n' (node number-received state)
        if len(received_string) == len(temp_packet) and received_string[3] == '-':
        #if True:
